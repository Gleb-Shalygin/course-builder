...
> **Проектные конвенции важнее этих общих рекомендаций.** Для файлов в `resources/js/components/**/*.vue` и `resources/js/composables/**/*.ts` сначала применяются скиллы [`vue-component-conventions`](../vue-component-conventions/SKILL.md) и [`vue-composable-conventions`](../vue-composable-conventions/SKILL.md) — например, в компонентах этого проекта блок `<template>` идёт перед `<script>`, что отличается от общего порядка ниже.
...
Generic Vue guidance keeps SFC sections in this order:
```
<script setup lang="ts">
<template>
<style scoped>
...
> В этом проекте порядок другой — см. [`vue-component-conventions`](../vue-component-conventions/SKILL.md): сначала `<template>`, потом `<script>`.
...
- SFC section order matches project convention (template → script → style — see `vue-component-conventions`).
